...
export function MediaActionModal({ 
  media, mediaType, onClose, onAddFull, onAddLocal, isLoadingMedia, smartNavigate 
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto media-scrollbar p-6">
          <AnimatePresence mode="wait">
            {view === 'main' && (
              <motion.div key="main" className="flex flex-col items-center gap-6">
            ...
            {view === 'seasons' && (
              <motion.div key="seasons" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col gap-4">
                    ...
                    {seasons.map((season) => (
                      <div key={season.number} className="group flex items-center justify-between p-4 bg-white/2 border border-white/5 rounded-2xl hover:bg-white/5 transition-all">
                        <div className="flex flex-col min-w-0">
                          <span className="text-[8px] font-black text-purple-main uppercase tracking-widest">
                ...
                <Button variant="link" icon={ChevronLeft} onClick={() => setView('main')} className="text-[10px] uppercase font-black text-white/20 mt-4">
                  Back to Selection
                </Button>
              </motion.div>
            )}
            ...
            {view === 'episodes' && (
              <motion.div key="episodes" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col gap-8">
                      ...
                      <div className="flex flex-col gap-2 flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <span className="px-1.5 py-0.5 bg-purple-main/20 border border-purple-main/30 text-purple-300 text-[8px] font-black uppercase rounded">
                              S{selectedSeason.number}
                            </span>
                            <span className="text-[10px] text-white/40 font-bold">
                              {selectedSeason.releaseDate?.split('-')[0]}
                            </span>
                          </div>
                          ...
                          >
                            ...
                            <span className="text-[8px] font-black uppercase tracking-tighter text-purple-300 group-hover:text-white">
                              Add Full Season
                            </span>
                          </button>
                        </div>
                        <p className="text-xs text-white/60 italic leading-relaxed line-clamp-3 font-light mt-1">
                          {selectedSeason.overview || "No description available for this season."}
                        </p>
                      </div>
                    </div>
                        ...
                        episodes.map((ep) => (
                          ...
                          >
                            <div className="relative w-32 aspect-video shrink-0 rounded-xl overflow-hidden border border-white/10 bg-black/20">
                              ...
                              />
                              <div className="absolute top-1.5 left-1.5 px-1.5 py-0.5 rounded bg-black/60 backdrop-blur-md border border-white/10 text-[8px] font-black text-purple-400">
                                EP {ep.number}
                              </div>
                            </div>
                            ...
                            <div className="flex flex-col justify-center flex-1 min-w-0">
                              <div className="flex items-center gap-3 text-[8px] font-bold uppercase tracking-widest text-white/30 mb-1">
                                <span className="flex items-center gap-1">
                                  <Clock size={10}/> 
                                  {ep.runtime > 0 ? `${ep.runtime}m` : 'N/A'}
                                </span>
                                <span>{formatDate(ep.releaseDate)}</span>
                              </div>
                              
                              <h5 className="text-[11px] font-black text-white uppercase truncate group-hover:text-purple-300 transition-colors">
                                {ep.title}
                              </h5>

                              <p className="text-[10px] text-white/40 line-clamp-2 mt-1 leading-relaxed font-medium italic">
                ...
                <Button variant="link" icon={ChevronLeft} onClick={() => setView('seasons')} className="text-[10px] uppercase font-black text-white/20">Back to Seasons</Button>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
