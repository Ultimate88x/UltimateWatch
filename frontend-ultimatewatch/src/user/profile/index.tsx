...
import { SquarePen, UploadCloud, LogOut, X, Calendar, Activity, Users } from "lucide-react";
    ...
    const fetchEvents = async () => {
      try {
        const response = await fetch(
          `http://localhost:3000/events/created?limit=3`,
            {
              method: 'GET',
              headers: {
                'Content-Type': 'application/json',
  ...
  return (
    <div className="relative w-full bg-blue-background flex justify-start items-start overflow-x-hidden p-10 gap-10">
      <div className="relative w-1/3 h-fit flex flex-col justify-start items-center">
        ...
        {!isUpdating && (
          <div className="mt-12 pt-8 border-t border-white/10 w-65">
            <div className="flex flex-col items-start gap-2">
              <h3 className="text-sm font-bold text-red-500 uppercase tracking-widest">Danger Zone</h3>
              <p className="text-xs text-white/40 leading-relaxed">Once you delete your account, there is no going back.</p>
              <Button type="button" variant="danger-outline" onClick={() => setIsModalOpen(true)} fullWidth className="mt-2 text-xs py-2">
                Delete Account
              </Button>
            </div>
          </div>
